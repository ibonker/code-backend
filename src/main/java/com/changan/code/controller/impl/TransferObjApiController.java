...
public class TransferObjApiController implements TransferObjApi {
	 ...
	 */
	@Override
	public ResponseEntity<ResultDTO> transferObjShowGet(@RequestParam String id) {
		// 通过id查询，获取TransferObj对象
		TransferObjPO transferObj = transferObjService.findTransferObjById(id);
		return new ResponseEntity<ResultDTO>(new ResultOfTransferObjDTO().transferObj(transferObj).message("成功")
	...
	public ResponseEntity<ResultDTO> transferObjSavePost(@RequestBody TransferObjPO transferObj) {
		...
		return new ResponseEntity<ResultDTO>(new ResultOfTransferObjDTO().transferObj(newTransferObj).message("成功")
				.statusCode(Constants.SUCCESS_API_CODE), HttpStatus.OK);
	}
	 ...
	 */
	@Override
	public ResponseEntity<ResultDTO> transferObjallShowGet(@RequestParam String ProjectId) {
		//获得查询结果
		List <TransferObjPO> transferObjs= transferObjService.findAllTransferObj(ProjectId);
		return new ResponseEntity<ResultDTO>(new ResultOfTransferObjDTO().transferObjs(transferObjs).message("成功").
				statusCode(Constants.SUCCESS_API_CODE),HttpStatus.OK);
	}
