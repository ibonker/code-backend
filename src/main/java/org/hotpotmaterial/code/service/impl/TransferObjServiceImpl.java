...
import org.hotpotmaterial.code.repository.TransferObjFieldRespository;
...
public class TransferObjServiceImpl implements ITransferObjService {
	...
	public TransferObjPO updateTransferObj(TransferObjPO transferObj) {
		// 当前DTO
		TransferObjPO updateTransferObj = transferObjRePo.findByIdAndDelFlag(transferObj.getId(),
	...
	public void deleteTransferObj(String id) {
		...
		TransferObjPO deleteTransferObj = transferObjRePo.findByIdAndDelFlag(id, Constants.DATA_IS_NORMAL);
	...
	public TransferObjPO findTransferObjById(String id) {
		...
		// 获取TransferObjField
		List<TransferObjFieldPO> transferObjField = transferObjFieldRePo.findByTransferObjIdAndDelFlag(id,
				Constants.DATA_IS_NORMAL);
		if (transferObj != null) {
			// 设置返回对象值
	...
	public List<TransferObjPO> findAllTransferObj(List<DatasourcePO> datasources, String projectId) {
		// 装入扫描到的Table对应的DTO
		List<TransferObjPO> transferObjs = new ArrayList<>();
		// 查询对应数据库中的表
		for (DatasourcePO datasourcePO : datasources) {
			List<TablePO> tables = tableService.findByDatasourceId(datasourcePO.getId(), projectId);
			// 根据需要扫描的tableID查询需要生成的DTO
			for (TablePO tablePO : tables) {
				TransferObjPO transferObjPO = transferObjRePo.findByProjectIdAndTableId(projectId, tablePO.getId());
				// 如果不为null则放入集合中
				if (transferObjPO != null) {
					transferObjs.add(transferObjPO);
				}
			}
		}
		// 生成自定义实体类DTO
		List<TransferObjPO> customClass = transferObjRePo.findCustomClass(projectId);
		if (customClass != null && customClass.size() > 0) {
			// 判断实体类是否有父类,父类是否被激活
			for (TransferObjPO childtransferObjPO : customClass) {
				String parentName = childtransferObjPO.getInheritObjName();
				// 存在父类
				if (parentName != null && !"".equals(parentName)) {
					// 判断Dto是否激活
					for (TransferObjPO parenttransferObjPO : transferObjs) {
						if (parentName.contains(parenttransferObjPO.getName())||parenttransferObjPO.getName().contains(parentName)) {
							transferObjs.add(childtransferObjPO);
							break;
						}
					}
				}else {
		...
		// transferObjs.addAll(customClass);
		return transferObjs;
	}
	...
	public Map<String, List<SimpleDataObj>> findClassnameByProjectId(String projectId) {
		// 根据项目ID查询对应的数据库
		List<DatasourcePO> datasources = datasourceRepository.findByProjectId(projectId);
		// 查询对应的DTO
		List<TransferObjPO> transferObjs = findAllTransferObj(datasources, projectId);
		Map<String, List<SimpleDataObj>> datamaps = Maps.newHashMap();
		for (TransferObjPO data : transferObjs) {
			// 生成新对象
			SimpleDataObj dataobj = new SimpleDataObj(data.getId(), data.getName(), data.getIsGeneric(),
					data.getPackageName(), data.getComments());
			// 根据packagename获取list
			List<SimpleDataObj> objLists = datamaps.get(data.getPackageName());
			if (null != objLists) {
				objLists.add(dataobj);
			} else {
				objLists = Lists.newArrayList();
				objLists.add(dataobj);
				datamaps.put(data.getPackageName(), objLists);
			}
		}
		// t.id as id, t.name as name, t.packageName as packageName, t.isGeneric as
		// isGeneric, t.comments as comments
		// List<Object[]> results = transferObjRePo.findClassNameByProjectId(projectId);
		/*
		 * Map<String, List<SimpleDataObj>> datamaps = Maps.newHashMap(); for (Object[]
		 * data : results) { // 生成新对象 SimpleDataObj dataobj = new SimpleDataObj((String)
		 * data[0], (String) data[1], (String) data[3], (String) data[2], (String)
		 * data[4]); // 根据packagename获取list List<SimpleDataObj> objLists =
		 * datamaps.get((String) data[2]); if (null != objLists) {
		 * objLists.add(dataobj); } else { objLists = Lists.newArrayList();
		 * objLists.add(dataobj); datamaps.put((String) data[2], objLists); } }
		 */
		return datamaps;
	}
	...
	public TransferObjPO createAutoCrudDTO(String projectId, String tableId, String tableName, String datasourcePName,
			String className) {
		String dtoName = "ResultOf".concat(CaseFormat.LOWER_UNDERSCORE.to(CaseFormat.UPPER_CAMEL, tableName))
				.concat("DTO");
		// 检查dto是否存在
		List<TransferObjPO> showPOs = transferObjRePo.findByNameAndPackageNameAndProjectIdAndDelFlag(dtoName,
				datasourcePName, projectId, Constants.DATA_IS_NORMAL);
		TransferObjPO showPO;
		if (showPOs.isEmpty()) {
			// 详情实体
			showPO = this.genTransferObjPO(projectId, tableId, dtoName, datasourcePName, className.concat("详情实体"));
			// 保存详情实体
			showPO = transferObjRePo.save(showPO);
			// 实体属性
			TransferObjFieldPO showPOField = this.genTransferObjFieldPO(showPO.getId(), tableName, "", BaseType.PO,
					className, tableName.concat("对象"));
	 ...
	 * @return
	 */
	private TransferObjPO genTransferObjPO(String projectId, String tableId, String dtoName, String packageName,
			String comments) {
	 ...
	 * @return
	 */
	private TransferObjFieldPO genTransferObjFieldPO(String transferObjId, String tableName, String postfix,
			BaseType type, String format, String description) {
	...
	private void generateDefaultDtoMaps() {
		...
		pagedtofields.add(new TransferObjFieldPO(BaseDTO.PageDTO.name(), "pageParms", BaseType.DTO.name().toLowerCase(),
				"PageParms", "分页参数实体"));
		pagedtofields.add(new TransferObjFieldPO(BaseDTO.PageDTO.name(), "orders", BaseType.ARRAY.name().toLowerCase(),
				"Order", "排序对象集合"));
		...
		resultschemadtofields.add(new TransferObjFieldPO(BaseDTO.ResultJsonSchemaDTO.name(), "jsonSchema",
				BaseType.DTO.name().toLowerCase(), "JsonSchema", "jsonSchema实体"));
